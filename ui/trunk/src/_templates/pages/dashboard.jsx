...
export default class PageDashboard extends Reflux.Component {
	constructor(props) {
		super(props)
	...
	hasFilterString() {
		const ttt = !(!this.state.filterColText1 && !this.state.filterColText2)
		const fff = !(!this.state.filterColText1 && !this.state.filterColText2 && !this.state.filterColText3 && !this.state.filterColText4 && !this.state.filterColText5 &&
			...
			if(!_.isNil(e.target.value) && !_.isEmpty(e.target.value) && e.target.value !== '') {
				if(e.target.id === 'column1' && this.state.showPriorityColumn === true) {
					if (e.target.value !== '' && e.target.value !== '0') {
						const filterRecords = this.state.application.filter(s => String(s.priority).toLowerCase().startsWith(e.target.value))
					...
					const filterRecords = this.state.application.filter(s => String(s.applicationId).toLowerCase().startsWith(e.target.value))
					this.setState({filterColText2: e.target.value, filterRecords: filterRecords})
				} else if (e.target.id === 'column3') {
					const filterRecords = this.state.application.filter(s => String(s.applicationName).toLowerCase().startsWith(e.target.value))
					this.setState({filterColText3: e.target.value, filterRecords: filterRecords})
				} else if (e.target.id === 'column4') {
					const filterRecords = this.state.application.filter(s => String(s.systemOwner).toLowerCase().startsWith(e.target.value))
					this.setState({filterColText4: e.target.value, filterRecords: filterRecords})
				} else if (e.target.id === 'column5') {
					const filterRecords = this.state.application.filter(s => String(s.iMatrixNumber).toLowerCase().startsWith(e.target.value))
					this.setState({filterColText5: e.target.value, filterRecords: filterRecords})
				} else if (e.target.id === 'column6') {
					const filterRecords = this.state.application.filter(s => String(s.poc).toLowerCase().startsWith(e.target.value))
					this.setState({filterColText6: e.target.value, filterRecords: filterRecords})
				} else if (e.target.id === 'column7') {
					const filterRecords = this.state.application.filter(s => String(s.phase).toLowerCase().startsWith(e.target.value))
					this.setState({filterColText7: e.target.value, filterRecords: filterRecords})
				} else if (e.target.id === 'column8') {
					const filterRecords = this.state.application.filter(s => String(s.status).toLowerCase().startsWith(e.target.value))
					this.setState({filterColText8: e.target.value, filterRecords: filterRecords})
				} else if (e.target.id === 'column9') {
					const filterRecords = this.state.application.filter(s => String(s.expiration).toLowerCase().startsWith(e.target.value))
		...
		if (event.target !== null) {
			...
			if(!hasErrors) {
				...
				if (this.state.newRecord) {
					...
					AppActions.updateApplication({
						applicationId: Number(this.state.recordField2),
						applicationName:this.state.recordField3, 
                        statusId: Number(this.state.recordField8), 
                        systemOwner:this.state.recordField4, 
                        iMatrixNumber: this.state.recordField5, 
    ...
    sortTable(event, sortKey) {
        ...
        if (!this.state.ascending) {
			...
			if(sortKey === this.state.columnPropertyName1) {
				this.setState({faSortColumn1: 'fa fa-sort-down'})
			} 
			else if(sortKey === this.state.columnPropertyName2) {
				this.setState({faSortColumn2: 'fa fa-sort-down'})
			} 
			else if(sortKey === this.state.columnPropertyName3) {
				this.setState({faSortColumn3: 'fa fa-sort-down'})
			} 
			else if (sortKey === this.state.columnPropertyName4) {
				this.setState({faSortColumn4: 'fa fa-sort-down'})
			} 
			else if (sortKey === this.state.columnPropertyName5) {
				this.setState({faSortColumn5: 'fa fa-sort-down'})
			}
			else if (sortKey === this.state.columnPropertyName6) {
				this.setState({faSortColumn6: 'fa fa-sort-down'})
			}
			else if (sortKey === this.state.columnPropertyName7) {
				this.setState({faSortColumn7: 'fa fa-sort-down'})
			}
			else if (sortKey === this.state.columnPropertyName8) {
				this.setState({faSortColumn8: 'fa fa-sort-down'})
			} 
			else if (sortKey === this.state.columnPropertyName9) {
				this.setState({faSortColumn9: 'fa fa-sort-down'})
			}  
 


			else if (sortKey === 'phaseId') {
				this.setState({faSortPhase: 'fa fa-sort-down'})
			}
			...
			if(sortKey === this.state.columnPropertyName1) {
				this.setState({faSortColumn1: 'fa fa-sort-up'})
			} 
			else if(sortKey === this.state.columnPropertyName2) {
				this.setState({faSortColumn2: 'fa fa-sort-up'})
			} 
			else if(sortKey === this.state.columnPropertyName3) {
				this.setState({faSortColumn3: 'fa fa-sort-up'})
			} 
			else if (sortKey === this.state.columnPropertyName4) {
				this.setState({faSortColumn4: 'fa fa-sort-up'})
			} 
			else if (sortKey === this.state.columnPropertyName5) {
				this.setState({faSortColumn5: 'fa fa-sort-up'})
			}
			else if (sortKey === this.state.columnPropertyName6) {
				this.setState({faSortColumn6: 'fa fa-sort-up'})
			} 
			else if (sortKey === this.state.columnPropertyName7) {
				this.setState({faSortColumn7: 'fa fa-sort-up'})
			}
			else if (sortKey === this.state.columnPropertyName8) {
				this.setState({faSortColumn8: 'fa fa-sort-up'})
			}  
			else if (sortKey === this.state.columnPropertyName9) {
				this.setState({faSortColumn9: 'fa fa-sort-up'})
			}  


			else if (sortKey === 'phaseId') {
				this.setState({faSortPhase: 'fa fa-sort-up'})
			}
